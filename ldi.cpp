//liar's dice improved
//intended to add options for ones wild
//and improve other stuff hopefully
//specifically the option to display dice while betting
//also delegating operations to functions as much as possible
//1 October 2019
//
...
int main(){
  ...
  cout << "Do you want 1s to be wild? 1 = yes, 0 = no";
  bool wild1s = false;
  cin >> wild1s;
    ...
    while(!called){//loop for rounds breaks when a call is made
      ...
      if(!input){//handling calling
        ...
        for(int j = 0; j < playercount; j++){
          ...
          for(int k = 0; k < playerdice[j]; k++){
            ...
            if(thisroll == prevbetface)
              checkcount++;
            else if(wild1s)//wild ones check
              if(thisroll == 1)
                checkcount++;
          }
          cout << endl;
        ...
        if(checkcount >= prevbetcount){
          cout << " has not lied! There are " << checkcount << ' ' << prevbetface << ".  Removing a die from the caller, player " << i + 1 << endl;
          playerdice[i]--;
        }else{
